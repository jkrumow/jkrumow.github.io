    ...
    function (exports, shader, framebuffer, data) {
        ...
        function scanlineStrokePolygon(vertices, polygon, color) {
            ...
            for (var v = 0; v < polygon.length; v++) {
                ...
                var nextVertexIndex = (v + 1) % polygon.length;
        ...
        function calculateLastDerivative(vertices, polygon) {
            for (var v = polygon.length - 1; v >= 0; v--) {
                var nextVertexIndex = (v + 1) % polygon.length;

                var currentVertex = polygon[v];
                var nextVertex = polygon[nextVertexIndex];

                var currY = Math.floor(vertices[currentVertex][1]);
                var nextY = Math.floor(vertices[nextVertex][1]);
        ...
        function assembleIntersectionForScanline(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
            ...
            lastDerivative = calculateLastDerivative(vertices, polygon);
            if (!lastDerivative) {
                return;
            }

            for (var v = 0; v < polygon.length; v++) {
                var currentVertexIndex = polygon[v];
                var vv = (v + 1) % polygon.length;
                var nextVertexIndex = polygon[vv];

                currX = Math.floor(vertices[currentVertexIndex][0]);
                currY = Math.floor(vertices[currentVertexIndex][1]);
                currZ = vertices[currentVertexIndex][2];

                nextX = Math.floor(vertices[nextVertexIndex][0]);
                nextY = Math.floor(vertices[nextVertexIndex][1]);
                nextZ = vertices[nextVertexIndex][2];

                startPoint = [currX, currY, currZ];
                endPoint = [nextX, nextY, nextZ];

                var edgeStartVertexIndex = currentVertexIndex;
                var edgeEndVertexIndex = nextVertexIndex;
                var edgeStartTextureCoord = null;
                var edgeEndTextureCoord = null;
                ...
                addIntersection(nextX, nextY, nextZ, 1, edgeStartVertexIndex, edgeEndVertexIndex, edgeStartTextureCoord, edgeEndTextureCoord);

                if (!lastDerivative) {
                    console.log("no last derivative.");
                }
        ...
        function interpolationPrepareScanline(startIntersection, endIntersection, texture) {
            ...
            interpolationData.dz = (zEndFill - zStartFill) / (xEndFill - xStartFill);
        ...
        function scanlineFillPolygon(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
            ...
            for (var y = 0; y < scanlineIntersection.length; y++) {
                ...
                if ((line.length < 2) || (line.length % 2)) {
                    console.log('Error in number of intersection in line: %s, %s', y, JSON.stringify(line, null, '\t'));
                    continue;
                }

                for (var l = 0; l < line.length; l += 2) {
                    var start = line[l];
                    var end = line[l + 1];

                    // Calculate interpolation variables for current scanline.
                    // Necessary for z-buffer, shading and texturing.
                    interpolationPrepareScanline(start, end);

                    for (var x = start.x; x <= end.x; x++) {
                        // Set z shorthand.
                        var z = interpolationData.z;
                        ...
                        if (zTest && horizontalClippingTest) {
                            ...
                            // framebuffer.set(x, y, null, color);
                            framebuffer.set(x, y, z, color);
                        }
        ...
        function calcPlaneEquation(vertices, polygon, normal) {
            ...
            D = -1 * vertice[0] * normal[0];

            // console.log("A %s B %s C %s D %s | iC %s AC %s", A, B, C, D, inverseC, AdivC);
        ...
        function getZ(x, y) {
            ...
            // if (!isFinite(z)) {
            //     console.log("z isNaN or not isFinite for (%s, %s): %s", x, y, z);
            // }
