...
define(["exports", "data", "glMatrix"], function (data, exports) {
	...
	function calcuateVertexNormalsFromPolygonNormals() {
		...
		for (var p = 0; p < this.polygonVertices.length; p++) {
			...
			for (var vv = 0; vv < polygon.length; vv++) {
				var vertNr = polygon[vv];

				// Accumulate/add all polygon normals.
				temp[vertNr].push(this.polygonNormals[p]);
			}
		}

		// Normalize normals.
		for (var nn = 0; nn < temp.length; nn++) {
			var tempNormal = temp[nn];
			var normal = [0, 0, 0];
			for (var nnn = 0; nnn < tempNormal.length; nnn++) {
				vec3.add(normal, tempNormal[nnn]);
			}
			vec3.normalize(normal);
			this.vertexNormals[nn] = normal;
		}
	...
	function calculateNormalForPolygon(vertices, polygon, n) {
		...
		for (var i = 0; i < polygon.length; i++) {
			...
			var k = (i + 2) % polygon.length;

			var vert0 = vertices[polygon[i]];
			var vert1 = vertices[polygon[j]];
			var vert2 = vertices[polygon[k]];
